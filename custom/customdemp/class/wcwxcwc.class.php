...
/**
 * \file        class/wcwxcwc.class.php
 * \ingroup     customdemp
 * \brief       This file is a CRUD class file for Wcwxcwc (Create/Read/Update/Delete)
 */
 ...
 * Class for Wcwxcwc
 */
class Wcwxcwc extends CommonObject
{
	/**
	 * @var string ID of module.
	 */
	public $module = 'customdemp';

	/**
	 * @var string ID to identify managed object.
	 */
	public $element = 'wcwxcwc';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'customdemp_wcwxcwc';
	...
	/**
	 * @var string String with name of icon for wcwxcwc. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'wcwxcwc@customdemp' if picto is file 'img/object_wcwxcwc.png'.
	 */
	...
	//  */
	// public $table_element_line = 'customdemp_wcwxcwcline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_wcwxcwc';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Wcwxcwcline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Wcwxcwc', 'fk_element'=>'fk_wcwxcwc'));
	...
	//  */
	// protected $childtablesoncascade = array('customdemp_wcwxcwcdet');

	// /**
	//  * @var WcwxcwcLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('customdemp', 'wcwxcwc', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('customdemp', 'wcwxcwc', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('customdemp', 'wcwxcwc_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'wcwxcwc/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'wcwxcwc/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'wcwxcwc/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'wcwxcwc/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->customdemp->dir_output.'/wcwxcwc/'.$oldref;
				$dirdest = $conf->customdemp->dir_output.'/wcwxcwc/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->customdemp->dir_output.'/wcwxcwc/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('customdemp','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('customdemp','customdemp_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'CUSTOMDEMP_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'CUSTOMDEMP_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'CUSTOMDEMP_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowWcwxcwc")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Wcwxcwc").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/customdemp/wcwxcwc_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$objectline = new WcwxcwcLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_wcwxcwc = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('CUSTOMDEMP_MYOBJECT_ADDON')) {
			$conf->global->CUSTOMDEMP_MYOBJECT_ADDON = 'mod_wcwxcwc_standard';
		}

		if (getDolGlobalString('CUSTOMDEMP_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('CUSTOMDEMP_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('CUSTOMDEMP_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/customdemp/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/customdemp/doc/";
...
/**
 * Class WcwxcwcLine. You can also remove this and generate a CRUD class for lines objects.
 */
class WcwxcwcLine extends CommonObjectLine
{
	// To complete with content of an object WcwxcwcLine
	// We should have a field rowid, fk_wcwxcwc and position
