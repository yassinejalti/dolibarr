...
/**
 *   	\file       wcwxcwc_list.php
 *		\ingroup    customdemp
 *		\brief      List page for wcwxcwc
 */
...
require_once __DIR__.'/class/wcwxcwc.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('customdemp', 'wcwxcwc', 'write');
	$permissiontodelete = $user->hasRight('customdemp', 'wcwxcwc', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("customdemp")) {
	accessforbidden('Module customdemp not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Wcwxcwc';
	$objectlabel = 'Wcwxcwc';
	$uploaddir = $conf->customdemp->dir_output;
...
$title = $langs->trans("Wcwxcwcs");
//$help_url = "EN:Module_Wcwxcwc|FR:Module_Wcwxcwc_FR|ES:Módulo_Wcwxcwc";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/customdemp/wcwxcwc_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-customdemp page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/customdemp/wcwxcwc_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendWcwxcwcRef";
$modelmail = "wcwxcwc";
$objecttmp = new Wcwxcwc($db);
