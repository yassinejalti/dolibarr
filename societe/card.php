...
// success notify for Tier
if ($_GET['id'] == "__ID__") {
	$result = $object->fetch($_GET['socid']);
	if ($result > 0) {
		$scNom = $object->name;
		setEventMessages('Le Tier "'.$scNom.'" a été créée avec succès', null);
	}
}

if ((int)$_GET['status'] == 4) {
	setEventMessages('success!', null);
}

// error notify for Tier
if ((int)$_GET['error'] == 4) {
	setEventMessages("fournisseur doit être Non si un client", null, 'errors');
}
